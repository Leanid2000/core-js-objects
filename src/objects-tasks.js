...
function mergeObjects(objects) {
  ...
  for (let i = 0; i < objects.length; i += 1) {
    const a = Object.entries(objects[i]);
    a.map((elem) => {
      elem.map((el, n, mass) => {
        if (n === 0 && !Object.keys(arr).includes(el)) {
          arr[el] = 0;
        } else if (n !== 0) {
          arr[mass[0]] += el;
        }
...
function compareObjects(obj1, obj2) {
  const a = Object.entries(obj1).flat(Infinity).join('');
  const b = Object.entries(obj2).flat(Infinity).join('');
  return a === b;
}
...
function makeWord(lettersObject) {
  const a = Object.values(lettersObject).flat(Infinity).fill(0);
  const b = Object.keys(lettersObject);
  const c = Object.values(lettersObject);
  b.map((el, n) => {
    c[n].map((elem) => {
      a[elem] = el;
      return elem;
...
function sellTickets(queue) {
  ...
  queue.map((elem) => {
    ...
    if (elem === 100) {
      if (arr.indexOf(50) > -1 && arr.indexOf(25) > -1) {
        arr.splice(arr.indexOf(25), 1);
        arr.splice(arr.indexOf(50), 1);
        arr.push(elem);
        arr.sort((a, b) => a - b);
      } else if (arr[0] === 25 && arr[1] === 25 && arr[2] === 25) {
        arr.splice(0, 3);
        arr.push(elem);
        arr.sort((a, b) => a - b);
      } else {
